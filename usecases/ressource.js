...
class Ressource{
    ...
    draw(){
           ...
           (this.data.children && !this.data.methods && !this.data.params)  
            ...
            if(this.data.children){
                this.data.children.map((m)=>{
                    aya.Ressource(this.x, this.y + 50, this.r, this.arc_angle, m);
                });
        }
        }
        ...
        if(this.data.children){
            this.data.children.map((m, index)=>{
                if(index == 0){
                    var dx = 300, dy = 200;
                    aya.Ressource(this.x - dx, this.y + dy, this.r, this.arc_angle, m);
                }
                else{
                    aya.Ressource(this.x + dx, this.y + dy, this.r, this.arc_angle, m);
                }
    ...
    drawPath(name){
        ...
        path.form.c_svg.setAttribute("fill", "#E7FBBE");
        console.log(name);
    ...
    drawRessource(){
        var x = this.x, y = this.y + this.r + 20, idx;

        this.circle = aya.Component("circle", {x: this.x, y: this.y, r: this.r});
        this.circle.form.c_svg.setAttribute("fill", "#D9D7F1");
        ...
        if(Object.keys(this.data)){
            ...
            text.c_svg.setAttribute("class", "name_res");
            text.title.textContent = "resource";
        }


        for(var m of methods){
            var arc = aya.Arc(this.x, this.y, x, y, this.arc_angle, 3.2/5);
